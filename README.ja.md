...
- 64bit の Windows 10 または Windows 11。
- .NET Framework 4.8 以上。Windows 10 22H2 と Windows 11 には互換性のある .NET Framework 4.x ランタイムが含まれます。
- リリースzipを実行するだけなら .NET Framework Developer Pack は不要です。

ソースからビルドする環境:

- Windows PowerShell または PowerShell 7。
- .NET Framework 4.8 Developer Pack、または .NET Framework 4.8 targeting pack を含む Visual Studio Build Tools。
- `build.ps1` は意図的に `C:\Windows\Microsoft.NET\Framework64\v4.0.30319\MSBuild.exe` を使います。これは .NET Framework 4.x 共通の CLR 4 ツールチェーンの場所であり、ターゲットが .NET Framework 4.0 という意味ではありません。

ビルドに必要なものは winget で入れられます:
...
`build.ps1` が .NET Framework 4.8 reference assemblies 不足を報告する場合は、
.NET Framework 4.8 Developer Pack を入れてください。これはソースからビルドする場合だけ必要で、
リリース済みの `Headroom.exe` を実行するだけなら不要です。
