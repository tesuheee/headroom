...
3. 初回のみ、各カードの **ログイン** ボタンを押します。
   ...
   - **CLI が入っていない場合**：既定ブラウザで OAuth ログインページが直接開きます。サインインするとタブに「ログイン完了」と表示され、Headroom が自動的に認証情報を取り込みます。
...
CLI がインストールされている場合は認証情報の書き込みを CLI に任せます。CLI が無い場合は
Headroom 自身が PKCE OAuth フロー（システムブラウザ + localhost コールバック）でログインし、
CLI 互換形式で認証ファイルに書き込みます。リフレッシュトークンを使ってアクセストークンを自動で
更新し続けるため、ログインのやり直しはほぼ発生しません。
