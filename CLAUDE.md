...
コードを書く前に、必ず以下を実行してください。

1. **タスク説明から触れるファイルを列挙する**
2. **モードを判定する**（下の運用モード定義を参照）
   - 標準モード対象ファイルに1つでも該当 -> 標準モード
   - 2つのエージェントが同時に作業する -> 分離モード
   - それ以外 -> 軽量モード
3. **判定結果を最初に宣言する**（実装より先に）
   - 例: `モード: 標準 - src/SettingsForm.cs を触るため`
4. **モードに応じたセットアップを行う**
   - 軽量: そのまま作業開始
   - 標準: `docs/ai/tasks/<task-name>.md` を作成し、`Owner`・`Scope`・`Out of scope` を記入してから作業開始
   - 分離: 可能なら適切なworktreeを作成または切り替える。環境上できない場合だけ、正確なworktreeコマンドを提示してユーザーの実行を待つ

セットアップが完了するまで実装を始めないでください。

ユーザーがモードを指定しない場合は、エージェントが作業内容から自動判定してください。判定が曖昧で、誤ると明確なリスクがある場合だけユーザーに確認します。

### タスク依頼の解決

ユーザーが具体的なファイル名を指定せずに「タスクを処理して」「対応して」「レビューして」「続きを進めて」と依頼した場合:

1. `docs/ai/tasks/` 配下のタスクファイルを探す。
2. 実行可能なタスクが1つだけなら、そのタスクを使う。
3. 実行可能なタスクが複数ある場合は、候補を列挙してどれを使うか確認する。
4. タスクファイルがなくても依頼内容だけで安全に進められる場合は、新しいタスクファイルを作成して適切なモードで続行する。
5. 安全にタスクを特定できない場合だけ、短く1つ確認質問をする。

実行可能なタスクとは、`Done`、`Closed`、`Cancelled` として明示されていないタスクを指す。

### タスクファイルの最小形

タスクファイルを作成する場合は、以下の形を使う。
...
標準または分離モードの作業終了時は、もう一方のエージェントへ渡す短い handoff を出してください。handoff はタスクファイルとPR/ブランチを参照し、詳細を全文再掲しないでください。
